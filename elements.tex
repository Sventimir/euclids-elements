...
\begin{theorem}
    ...
    \begin{enumerate}
        ...
        \item Skonstruować trójkąt równoboczny \(\triangle DEF\) o podstawie
            \(DE\) \prop{1.1}.
        \item Prosta linia \(CF\) jest protopadła do \(AB\) \post{1.1}.
    \end{enumerate}
...
\begin{theorem}
    W dowolnym trójkącie, dłuższy bok znajduje się naprzeciw większego
    kąta\footnote{
...
draw_lines(b, d, a, b, c, d)
    ...
    \begin{proof}
        Niech \(AC\) będzie najdłuższym bokiem \(\triangle ABC\). Obierzmy
        punkt \(D\) na boku \(AC\) taki że \(|AD| = |AB|\) \prop{1.3}. Połączmy
        ...
        Analogicznie można pokazać, że \(|\measuredangle BAD| < |\measuredangle
        ABC|\).
    \end{proof}
\end{theorem}
